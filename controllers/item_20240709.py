    ...
    if submit_btn:
        ...
        if (item_id != '' and des != '' and category_id != '' and category_id_sp != '' and unit_type != '' and manufacturer != '' and conv_factor > 0.00 and price > 0.00 and status != '' and field1 != '' and note != ''):
            ...
            if len(item_check) != 0:
                ...
                insert_item_sql = f"INSERT INTO sm_item (cid, item_id, name, pack_size, des, category_id, unit_type, manufacturer, item_carton, conv_factor, price, dist_price, vat_amt, total_amt, status, field1, note, created_on, created_by) VALUES ('{cid}','{str(item_id)}','{str(des)}','{str(pack_size)}','{str(des)}','{str(category_id)}','{str(unit_type)}','{str(manufacturer)}','{str(item_carton)}','{str(price)}','{str(dist_price)}','{str(vat_amt)}','{str(total_amt)}','{str(status)}','{str(field1)}','{str(note)}','{date_fixed}','{user_id}');"
...
def item_edit():
    ...
    if update_btn:
        ...
        field2_up = int(request.vars.conv_factor_input) if request.vars.conv_factor_input else None
