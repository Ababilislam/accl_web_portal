def rep():
    ...
    rep_condition = ''
#   checking filter session hold or not  its holding session vlaue.
    session.search_id_name=""
    ...
    if submit_btn:
        ...
        # return rep_id, rep_name, rep_mobile, rep_pass, rep_status, rep_user_type
        ...
        if (rep_id !="" and rep_name !=""and rep_user_type !="" and rep_mobile !=""and rep_pass !="" and rep_status !=""):
            ...
            if len(check_rep)>0:
                ...
                session.flash = 'Representative added successfully'
    ...
    if filter_btn:
        search_id_name = str(request.vars.search_id_name)
        search_value = str(request.vars.search_value)
        # print(select_optn)
        if search_id_name:
            try:
                rep_id = search_id_name.split('|')[0].strip()
                # rep_name = search_id_name.split('|')[1].strip()
                # return rep_id
                if session.search_id_name !="" or search_id_name != "":
                    rep_condition = f" and (rep_id = '{rep_id}') "
                session.search_id_name=search_id_name
    ...
    # return session.rep_condition
    get_rep_list_sql = f"SELECT rep_id, name, mobile_no, status, user_type, password FROM sm_rep WHERE cid = '{cid}' {rep_condition} and user_type ='rep' ORDER BY id DESC LIMIT %d, %d;" % limitby 
    rep_records = db.executesql(get_rep_list_sql, as_dict = True)
    # return get_rep_list_sql
    total_rep_sql = f"SELECT * FROM sm_rep WHERE cid = '{cid}' {rep_condition} and user_type = 'rep' ORDER BY id DESC;"
...
def rep_edit():
    ...
    rep_id = str(request.args(0)).strip().replace('_',' ')
    ...
    if btn_upload=='Upload':
        ...
        for i in range(total_row):
            ...
            if len(coloum_list)!=5:
                ...
                rep_name = str(coloum_list[1]).strip().upper()
                mobile = str(coloum_list[2]).strip().upper()
                type = "rep"
                pswrd = str(coloum_list[3]).strip().upper()
                status = str(coloum_list[4]).strip().upper()
                # pswrd = random.randint(0000,9999)
                # return f"sup:{sup_id} sup_name:{sup_name} mobile:{mobile} type:{type} status:{status} pswrd:{pswrd}"
                if (rep_id==None or rep_id=='') or (rep_name == None  or rep_name== '') or (mobile == None  or mobile== '') or (status == None  or status== '') or (pswrd == None or pswrd ==''):
                    ...
                    existCheckRows= " select * FROM sm_rep WHERE cid='"+str(cid)+"' and rep_id = '"+str(rep_id)+"' and user_type LIKE 'rep' LIMIT 0,1"
                    # return existCheckRows
                    existCheck = db.executesql(existCheckRows)

                    if len(existCheck) > 0:
                        error_data=row_data+'(Duplicate Representative please check!)\n'
                        error_str=error_str+error_data
                        count_error+=1
                        continue
                    else:
                        try:
                            insert_sql =f"INSERT INTO sm_rep (cid, rep_id, name, mobile_no, password, status, user_type) VALUES ('{cid}','{rep_id}','{rep_name}','{mobile}','{pswrd}','{status}','{type}');"
                            # return insert_sql
...
def rep_area():
    ...
    # session filter session hold ro not session is holding for filter.
    ...
    if add_btn:
        ...
        if rep_input == '':
            response.flash = 'Select MSO'
        elif area_input == '':
            response.flash = 'Select Territory'
        else:
            try:
                rep_id, rep_name = str(rep_input).strip().upper().split('|')
            except ValueError:
                response.flash = 'Invalid MSO Format'
            else:
                try:
                    area_id, area_name = str(area_input).strip().upper().split('|')
                except ValueError:
                    response.flash = 'Invalid Territory Format'
                else:
                    check_rep_sql = f"SELECT * FROM sm_rep_area WHERE cid = '{cid}' AND rep_id = '{rep_id}' AND area_id = '{area_id}' LIMIT 1;"
    ...
    # for download
    session.download_condition=filter_condition
...
def rep_input_list():
    ...
    replistRows_sql = "select rep_id ,name from sm_rep where cid = '"+c_id+"' and user_type='rep' group by rep_id;"
...
def download_rep_area():
    ...
    download_sql = f"SELECT id,rep_id, rep_name, area_id,area_name FROM sm_rep_area WHERE cid = '{cid}' {session.download_condition} ORDER BY id DESC;"
        ...
        for i in range(total_row):
            ...
            if len(coloum_list)!=2:
                ...
                if rep_id=='NONE'or territory_id == 'NONE' or rep_id=='' or territory_id== '':
                    ...
                    existCheckRows= " select * FROM sm_rep_area WHERE cid='"+str(cid)+"' and rep_id = '"+str(rep_id)+"' and area_id = '"+str(territory_id)+"' LIMIT 0,1"
                    # return existCheckRows
                    existCheck = db.executesql(existCheckRows)

                    if len(existCheck) > 0:
                        error_data=row_data+'(Duplicate MSO Please Check)\n'
                        error_str=error_str+error_data
                        count_error+=1
                        continue
                    else:
...
def download_representative():
    ...
    get_rep_list_sql = f"SELECT rep_id, name, mobile_no, status, user_type, password FROM sm_rep WHERE cid = '{cid}' {rep_condition} and user_type='rep' ORDER BY rep_id DESC;"
    # return get_rep_list_sql
...
def get_rep_id_name_list_rep():
    rep_id_names_Str = ''
    cid = session.cid
    if (cid == '' or cid == None):
        redirect(URL('default','index'))
    
    sql_query = f"select rep_id, name from sm_rep  where cid = '{cid}' and user_type='rep' group by rep_id order by rep_id;"
    rows = db.executesql(sql_query, as_dict = True)
    ...
    for idx in range(len(rows)):
        ...
        if rep_id_names_Str == '':
            rep_id_names_Str = rep_id + '|' + name 
        else:
            rep_id_names_Str += ',' + rep_id + '|' + name

    return rep_id_names_Str


def get_rep_status_list():
    rep_status_Str = ''
    cid = session.cid
    if (cid == '' or cid == None):
        redirect(URL('default','index'))
    
    sql_query = f"select status from sm_rep  where cid = '{cid}' group by status;"
    rows = db.executesql(sql_query, as_dict = True)

    for idx in range(len(rows)):
        name = str(rows[idx]['status']).replace('|', ' ').replace(',', ' ')
