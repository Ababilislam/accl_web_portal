...
def item_screen():
    ...
    if submit_btn:
        item_id = str(request.vars.item_id_input).upper().replace(' ','_')
        # name = str(request.vars.item_name_input).upper()
        # pack_size = str(request.vars.pack_size_input)
        pack_size = ''
        des = str(request.vars.desc_input).upper()
...
def item_edit():
    ...
    if update_btn:
        item_id_up = str(request.vars.item_id_input).upper()
        name_up = str(request.vars.item_name_input).upper()
        pack_size_up = str(request.vars.pack_size_input)
        des_up = str(request.vars.desc_input).upper()
        category_id_up = str(request.vars.cat_id_input).upper()
        category_id_sp_up = str(request.vars.cat_id_sp_input).upper()
        unit_type_up = str(request.vars.unit_type_input).upper()
        manufacturer_up = str(request.vars.manufacturer_input).upper()
        ...
        status_up = str(request.vars.status_input).upper()
        field1_up = str(request.vars.field1_input).upper()
        field2_up = int(request.vars.field2_input) if request.vars.field2_input else None
        note_up = str(request.vars.note_input).upper()
