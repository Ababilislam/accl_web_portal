...
def supervisor_create():
    ...
    if submit_btn:
        ...
        sup_user_type = str(request.vars.rep_type).upper()
        user_type = "sup"
        ...
        if (sup_id !="" and sup_name !=""and sup_user_type !="" and sup_mobile !=""and sup_pass !="" and sup_status !=""):
            ...
            if len(check_sup)>0:
                ...
                insert_sup_sql = f"INSERT INTO sm_rep (cid, rep_id, name, mobile_no, password, status, note, user_type) VALUES ('{cid}','{sup_id}','{sup_name}','{sup_mobile}','{sup_pass}','{sup_status}','{sup_user_type}','{user_type}');"
...
def supervisor_edit():
    ...
    # return select_sup_record_sql
    sup_record = db.executesql(select_sup_record_sql, as_dict = True)
    # return response.json(sup_record)
    ...
    if update_btn:
        ...
        sup_user_type_up = str(request.vars.rep_type).upper()
        ...
        update_sup_sql = f"UPDATE sm_rep SET name = '{sup_name_up}', mobile_no = '{sup_mobile_up}', password = '{sup_pass_up}', status = '{sup_status_up}', note = '{sup_user_type_up}' WHERE cid='{cid}' and rep_id='{(str(sup_id))}' limit 1;"
    ...
    if delete_btn:
        ...
        redirect(URL('supervisor','supervisor_create'))
    return dict(sup_id=sup_id, sup_name=sup_name,sup_mobile=sup_mobile, sup_pass=sup_pass,rep_status=sup_status,rep_user_type=sup_user_type)
    ...
    if btn_upload=='Upload':
        ...
        for i in range(total_row):
            ...
            if len(coloum_list)!=5:
                ...
                # return f"sup:{sup_level} sup_name:{sup_name} mobile:{mobile} type:{type} status:{status} pswrd:{pswrd}"
                if (sup_id==None or sup_id=='') or (sup_name == None  or sup_name== '') or (mobile == None  or mobile== '') or (type == None  or type== '') or (status == None  or status== '') :
                    ...
                    if len(existCheck) > 0:
                        ...
                        try:
                            insert_sql =f"INSERT INTO sm_rep (cid, rep_id, name, mobile_no, password, status, note, user_type) VALUES ('{cid}','{sup_id}','{sup_name}','{mobile}','{pswrd}','{status}','{type}','sup');"
