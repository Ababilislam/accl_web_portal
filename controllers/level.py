    ...
    if btn_search:
        ...
        if level_search == "" or level_search == None:
           session.search_error='Please select level !'
            ...
            try:
                level_id,level_name,level0=level_search.split('|')
                # return level_id,level_name,level0
                search_level_sql = f"select * from sm_level where level_id ='{level_id}' and level_name='{level_name}' and level0='{level0}' limit 1;"
                # return search_level_sql
                try:
                    ...
                    if depth == 0:
                        ...
                        # get_level_list_sql = f"SELECT * FROM `sm_level` WHERE cid='{c_id} and`depth` = 0 GROUP BY level0;"
                        get_level_list_sql = f"SELECT * FROM `sm_level` WHERE cid='{c_id}' and depth ='{depth}' GROUP BY level0;"
                    ...
                    if depth == 2:
                        ...
                        get_level_list_sql = f"SELECT * FROM sm_level WHERE cid='{c_id}' and parent_level_id = '{parent_id}' and level0='{division_id}' GROUP BY level2,level1,level0;"
                        # return get_level_list_sql
                        level_records = db.executesql(get_level_list_sql, as_dict = True)
                    ...
                    if depth == 3:
                        ...
                        get_level_list_sql = f"SELECT * FROM sm_level WHERE cid='{c_id}' and parent_level_id = '{parent_id}' and level0='{division_id}' GROUP BY level3, level2,level1,level0;"
                        # return get_level_list_sql
                        level_records = db.executesql(get_level_list_sql, as_dict = True)
    ...
    if depth == 0:
        ...
        if submit_btn:
            if division_id == '' or division_id == 'None' or division_id==None:
                ...
                session.insert_success='Successfully saved!'
        ...
        # return division_id, parent_name
        if submit_btn:
        ...
        # return division_id, parent_id, parent_name
        if submit_btn:
...
def delete_record(depth,level_id,item_id):
    ...
    if child_level:
       session.delete_error='Delete child level first !'
    else:
...
def get_level_list():
    ...
    get_rows = f"SELECT level_id, level_name, level0, level0_name FROM sm_level WHERE cid='{cid}' ORDER BY level_name;"
    # return get_rows
    rows = db.executesql(get_rows,as_dict=True)
    # return db._lastsql
    for row in rows:
        level_id = str(row['level_id'])
        name = str(row['level_name']).replace('|', ' ').replace(',', ' ')
        lvl_0 = str(row['level0']).replace('|', ' ').replace(',', ' ')
        # return level_id
        if resStr == '':
            resStr = level_id + '|' + name + '|' +lvl_0
        else:
            resStr += ',' + level_id + '|' + name+ '|' +lvl_0
