...
def item_screen():
    ...
    if submit_btn:
        ...
        total_amt = float(request.vars.total_amt_input) if request.vars.total_amt_input else None
        ...
        # and category_id_sp != '', and field2 > 0
        if (item_id != '' and name != '' and pack_size != '' and des != '' and category_id != '' and unit_type != '' and manufacturer != '' and item_carton > 0 and price > 0.00 and dist_price > 0.00 and vat_amt > 0.00 and status != '' and total_amt > 0 and field1 != '' and note != ''):
            item_check_sql = f"SELECT item_id, name FROM sm_item WHERE cid='ACCL' AND item_id='{str(item_id)}' AND name='{str(name)}';"
            item_check = db.executesql(item_check_sql)

            if len(item_check) != 0:
                response.flash = "This item already exists!"
            else:
                insert_item_sql = f"INSERT INTO sm_item (cid, item_id, name, pack_size, des, category_id, unit_type, manufacturer, item_carton, price, dist_price, vat_amt, total_amt, status, field1, note, created_on, created_by) VALUES ('{cid}','{str(item_id)}','{str(name)}','{str(pack_size)}','{str(des)}','{str(category_id)}','{str(unit_type)}','{str(manufacturer)}','{str(item_carton)}','{str(price)}','{str(dist_price)}','{str(vat_amt)}','{str(total_amt)}','{str(status)}','{str(field1)}','{str(note)}','{date_fixed}','{user_id}');"
    ...
    if filter_btn:
        ...
        if session.item_id_name != '' or item_id_name != '':
            condition += f"AND item_id = '{str(item_id_name)}'"
    ...
    if session.condition == '' and session.condition != 'None' and session.condition != None:
        ...
        all_item_sql = f"SELECT * FROM sm_item WHERE cid='ACCL' {condition} LIMIT {start_index}, {page_limit};"

    item_rec = db.executesql(all_item_sql, as_dict=True)

    session.condition = condition

    total_record_sql = f"SELECT COUNT(id) AS total FROM sm_item WHERE cid='ACCL' {condition} ORDER BY id ASC;"
...
def item_edit():
    ...
    if update_btn:
        ...
        update_sql = f"UPDATE sm_item SET item_id = '{str(item_id_up)}', name = '{str(name_up)}',  pack_size = '{str(pack_size_up)}', des = '{str(des_up)}', category_id = '{str(category_id_up)}', category_id_sp = '{str(category_id_sp_up)}', unit_type = '{str(unit_type_up)}', manufacturer = '{str(manufacturer_up)}', item_carton = '{str(item_carton_up)}', price = '{str(price_up)}', dist_price = '{str(dist_price_up)}', vat_amt = '{str(vat_amt_up)}', total_amt = '{str(total_amt_up)}', status = '{str(status_up)}', field1 = '{str(field1_up)}', field2 = '{str(field2_up)}', note = '{str(note_up)}', updated_on = '{date_fixed}', updated_by = '{user_id}' WHERE item_id = '{str(item_id)}' LIMIT 1;"
