def rep():
    ...
    if request.vars.btn_filter is not None and len(request.vars.search_value) > 0:
        
        if request.vars.search_type == 'RepID':
            data = request.vars.search_value.strip().split('|')
            filter_rep_id = ''
            filter_name = ''
            filter_mobile = ''

            if len(data) > 0:
                filter_rep_id = data[0]
                filter_name = filter_rep_id
                filter_mobile = filter_rep_id
            if len(data) >= 2:
                filter_name = data[1]
                filter_mobile = filter_name
            if len(data) == 3:
                filter_mobile = data[2]
            if filter_rep_id != '':
                condition = f" and (rep_id = '{filter_rep_id}' or name like '%{filter_name}%' or mobile_no = '{filter_mobile}') "
        if request.vars.search_type == 'Status':
            condition = f" and status = '{request.vars.search_value.strip()}' "
        session.filter_rep_sql_condition = condition

    if request.vars.btn_rep_all:
        condition = ''
        session.filter_rep_sql_condition = condition

    get_rep_list_sql = f"SELECT rep_id, name, mobile_no, status, user_type, password FROM sm_rep WHERE cid = '{cid}' {condition} ORDER BY id ASC LIMIT {start_index}, {page_limit};"
...
def download_representative():
    ...
    if session.filter_rep_sql_condition is not None and len(session.filter_rep_sql_condition) > 0:
        condition = session.filter_rep_sql_condition
        
    
    get_rep_list_sql = f"SELECT rep_id, name, mobile_no, status, user_type, password FROM sm_rep WHERE cid = '{cid}' {condition} ORDER BY rep_id ASC;"
