...
function check_user() {	
	...
	// var  apipath_base_photo_dm='http://w05.yeapps.com/dmpath/dmpath_syncmobile_pocketmarket/get_path?CID='+cid +'&HTTPPASS=e99business321cba'

	var  apipath_base_photo_dm='http://127.0.0.1:8000/hamdard/syncmobile_pocketmarket/dmpath?CID='+cid +'&HTTPPASS=e99business321cba'
...
//							--------------------------
							...
							$.ajax(localStorage.base_url+'check_user_pharma?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&synccode='+localStorage.synccode+'&version=16',{
								// cid:localStorage.cid,rep_id:localStorage.user_id,rep_pass:localStorage.user_pass,synccode:localStorage.synccode,
								type: 'POST',
								timeout: 30000,
								error: function(xhr) {
								...
								success:function(data, status,xhr){	
									 ...
									 if (status!='success'){
										...
										if (resultArray[0]=='FAILED'){
											...
											var clPocketMarket_idName=result_string.split('<fdfd>')[17]
											// alert (clPocketMarket_idName)
											$("#dCId").val(client_id)
											...
											$("#dCStatus").val(Status)
											// $("#cl_pocketMarket_edt").val(Status)
														...
														alert ('Network Timeout. Please check your Internet connection')
													 	...
													 	var resultArray = data.replace('</START>','').replace('</END>','').split('<SYNCDATA>');	
																...
																if (resultArray[0]=='FAILED'){
																	...
																	$('#cl_pocketMarket_edt').empty();
																	var opt='<option value="'+clPocketMarket_idName+'" selected>'+clPocketMarket_idName+'</option>'
																	$('#cl_pocketMarket_edt').append(opt);
																	// $('#cl_pocketMarket_edt').append(opt);
																	for (var s=0; s < resultList.length; s++){
																		var opt='<option value="'+resultList[s]+'">'+resultList[s]+'</option>'
																		 $('#cl_pocketMarket_edt').append(opt);
...
function chemistProfileSubmit() {
	...
	var cl_pocketMarket_edt=$("#cl_pocketMarket_edt").val()//Microunion
	// alert(cl_pocketMarket_edt)
	var error_flag=0
...
function checkInbox() {	
		  ...
		  $.ajax(localStorage.report_url+'checkInbox?cid='+localStorage.cid+'&rep_id='+localStorage.user_id+'&rep_pass='+localStorage.user_pass+'&synccode='+localStorage.synccode+'&version=16',{
			 ...
			 if (status!='success'){
				alert ('Network Timeout. Please check your Internet connection')
			 	...
			 	var resultArray = data.replace('</START>','').replace('</END>','').split('<SYNCDATA>');	
						...
						if (resultArray[0]=='FAILED'){
							$("#error_chemist_add_page").text(resultArray[1]);	
							...
							$('#cl_pocketMarket').empty();
							for (var s=0; s < resultList.length; s++){
								var opt='<option value="'+resultList[s]+'">'+resultList[s]+'</option>'
								 $('#cl_pocketMarket').append(opt);
...
function chemist_submit() {	
	...
	var cl_pocketMarket=$("#cl_pocketMarket").val()
	// alert(cl_pocketMarket)
